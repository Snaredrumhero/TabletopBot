namespace Table_Top_Bot
{
    internal static class XPStorage
    {
        ...
        public class User
        {
            ...
            public static bool operator > (User _a, User _b) { return _a.GetPointValue() > _b.GetPointValue(); }
            public static bool operator <(User _a, User _b) { return _a.GetPointValue() < _b.GetPointValue(); }

            public string Print() { return $"{discordID}\nPID: {PID}\nPoints: {GetPointValue()}"; }
            //Game type 1 = ranked, 2 = CoOp, 3 = Teams, 4 = Party
            //Rank = 1 for win 2 for loss
            public void UserAddGame(uint _playerCount, uint _gameType, uint _rank, uint _length)
            {
            ...
            public void UserRemoveGame(int _id)
            {
                for (int i = 0; i < gamesPlayed.Count; i++)
                    if (gamesPlayed[i].id == _id)
                    {
                        gamesPlayed.RemoveAt(i);
                        return;
                    }
            }
            ...
            public void ClaimAchivement(string _achievementName)
            {
                if (Achievements.ContainsKey(_achievementName))
                    achievements[Achievements.Keys.ToList().IndexOf(_achievementName)] = true;
            }
            public void UnclaimAchievement(string _achievementName)
            {
                if (Achievements.ContainsKey(_achievementName))
                    achievements[Achievements.Keys.ToList().IndexOf(_achievementName)] = false;
            }
        ...
        public static string DisplayTopXUsers(int _x)
        {
            ...
            string s = "";
            for (int i = 0; i < _x; i++)
                s += $"{i}: {Users[i].GetPointValue()} - {Users[i].discordID}\n";
            return s;
        }
    }
}